...
{	
	...
	int GC_and_WL_Unit_Page_Level::write_pages()
	{
		...
		if (waiting_writeback_transaction.size() != 0) {
			...
			CustomIterator first (waiting_writeback_transaction.begin());
			CustomIterator second (waiting_writeback_transaction.end());

			stable_sort(first, second, cmp_gc);
			waiting_writeback_transaction = list<NVM_Transaction*>(first, second);
	...
	int GC_and_WL_Unit_Page_Level::read_pages()
	{
		...
		CustomIterator first (waiting_submit_transaction.begin());
		CustomIterator second (waiting_submit_transaction.end());
		stable_sort(first, second, cmp_gc);
		waiting_submit_transaction = list<NVM_Transaction*>(first, second);
